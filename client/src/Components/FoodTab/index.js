...
import { IoBeerOutline, IoNutritionOutline } from "react-icons/io5";
...
const [allTypes, setAllTypes] = useState([
  ...
  return (
      <>
        <div className="lg:hidden bg-white shadow-lg fixed bottom-0 p-3 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border ">
          {allTypes.map((items) => (
            <Link to={`/${items.id}`}>
              <div
                className={
  ...
  return (
    <>
      <div className="hidden lg:flex gap-14 container px-20 my-8 mx-10 absolute top-16 mr-0">
        {allTypes.map((items) => (
          <Link to={`/${items.id}`}>
            <div
